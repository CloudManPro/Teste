...
resource "aws_iam_role_policy_attachment" "policy_Function2_st_State3_attach" {
  ...
  role                              = aws_iam_role.role_Function2.name
}
...
resource "aws_api_gateway_rest_api" "RestAPI1" {
  ...
  depends_on                        = [aws_iam_role.role_apigw_RestAPI1_to_my-bucket-aghjklkksjj, aws_iam_role.role_apigw_RestAPI1_to_Queue, aws_iam_role.role_apigw_RestAPI1_to_TableOpenAPI, aws_iam_role.role_apigw_RestAPI1_to_TableO2]
}
