...
data "archive_file" "archive_CloudManMain_Function20" {
  output_path                       = "${path.module}/CloudManMain_Function20.zip"
...
resource "aws_lambda_function" "Function20" {
  ...
  architectures                     = ["arm64"]
  filename                          = "${data.archive_file.archive_CloudManMain_Function20.output_path}"
  ...
  runtime                           = "python3.13"
  source_code_hash                  = "${data.archive_file.archive_CloudManMain_Function20.output_base64sha256}"
  timeout                           = 30
  environment {
    variables                       = {
